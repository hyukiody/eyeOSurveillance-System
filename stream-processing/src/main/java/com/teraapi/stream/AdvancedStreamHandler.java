...
public class AdvancedStreamHandler implements HttpHandler {
    ...
    private final AdvancedEncryption.AESGCMEncryption aesGcm;
    private SecretKey aesKey;

    public AdvancedStreamHandler(int requestsPerMinute) throws Exception {
        this.rateLimiter = new RateLimiter(requestsPerMinute);
        this.aesGcm = new AdvancedEncryption.AESGCMEncryption();
        this.aesKey = AdvancedEncryption.generateAESKey();
    }
    ...
    private void handleGCMEncrypt(HttpExchange exchange) throws Exception {
        ...
        String encrypted = AdvancedEncryption.AESGCMEncryption.encrypt(plaintext, aesKey);
        sendResponse(exchange, 200, "{\"encrypted\":\"" + encrypted + "\",\"algorithm\":\"AES-GCM-256\"}");
    }

    private void handleGCMDecrypt(HttpExchange exchange) throws Exception {
        String body = new String(exchange.getRequestBody().readAllBytes(), StandardCharsets.UTF_8);
        String encrypted = extractJson(body, "encrypted");

        String decrypted = AdvancedEncryption.AESGCMEncryption.decrypt(encrypted, aesKey);
        sendResponse(exchange, 200, "{\"decrypted\":\"" + decrypted + "\"}");
    }
    ...
    private void handleRSAEncrypt(HttpExchange exchange) throws Exception {
        ...
        sendResponse(exchange, 200, "{\"encrypted\":\"" + encrypted + "\",\"algorithm\":\"RSA-2048\"}");
    }
